...
describe("Part Parts", function() {
	...
	it("connects parts", function(done) {
		var partpart = testhelpers.create_partpart()
		partpart.connect(partpart.parts[0].outputs[0], partpart.parts[1].inputs[0])
	...
	it("connects parts of parts of parts", function(done) {
		...
		partpart1.connect(partpart1.parts[0].outputs[0], partpart1.parts[1].inputs[0])
		var partpart2 = testhelpers.create_partpart()
		partpart2.connect(partpart2.parts[0].outputs[0], partpart2.parts[1].inputs[0])
		var partpart3 = new core.PartPart("Mega Adder", [partpart1, partpart2])
		partpart3.connect(partpart3.parts[0].outputs[0], partpart3.parts[1].inputs[0])
	...
	it("disconnects parts", function() {
		...
		partpart.inputs[1].fill(2)
		partpart.inputs[2].fill(3)
		partpart.disconnect(partpart.parts[0].outputs[0], partpart.parts[1].inputs[0])
		expect(partpart.inputs.length).toBe(4)
		expect(partpart.outputs.length).toBe(2)
	})
	...
	it("can connect one output to two inputs", function(done) {
		var partpart = testhelpers.create_partpart()
		partpart.connect(partpart.parts[0].outputs[0], partpart.parts[1].inputs[0])
		partpart.connect(partpart.parts[0].outputs[0], partpart.parts[1].inputs[1])
